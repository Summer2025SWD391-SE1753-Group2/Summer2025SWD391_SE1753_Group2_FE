        ...
        if (tags.length > 0) {
          if (tags.length === 1) {
            ...
            const tagPromises = tags.map((tag) =>
              searchPostsByTag(tag, skip, postsPerPage)
            );
            const tagResults = await Promise.all(tagPromises);
            // Flatten and remove duplicates
            postResults = Array.from(
              new Map(
                tagResults.flat().map((post) => [post.post_id, post])
              ).values()
            );
          }
        }
      ...
      {isLoading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[...Array(8)].map((_, index) => (
            <Card
              key={index}
              className="overflow-hidden rounded-xl border border-gray-200 shadow-sm bg-gray-100 animate-pulse"
            >
          ...
          {users.length > 0 &&
            users.map((user) => (
              <Link
                ...
                key={user.account_id}
              >
                <Card className="overflow-hidden rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300 bg-white cursor-pointer">
          ...
          {posts.length > 0 &&
            posts.map((post) => (
              <Link to={`/posts/${post.post_id}`} key={post.post_id}>
                <Card className="overflow-hidden rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300 bg-white relative cursor-pointer">
                  ...
                  <CardContent className="p-3 flex flex-col items-center relative z-10 bg-white bg-opacity-95 pt-48">
                    <h3 className="font-semibold text-center text-lg text-gray-800 line-clamp-2">
                      {post.title}
                    </h3>
                    <p className="text-sm text-gray-500 text-center mt-1">
